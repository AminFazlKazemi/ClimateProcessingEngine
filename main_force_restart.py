...
# -*- coding: utf-8 -*-
"""
main_force_restart.py - شروع اجباری از بلوک ۰ برای پردازش tmin و tmax
"""
...
# checkpoint_manager
# ============================================================================
from checkpoint_manager import reset_checkpoint, ensure_checkpoint, save_checkpoint, delete_checkpoint

# ============================================================================
# outlier logger
# ============================================================================
from monitoring.outlier_logger import clear_outlier_log, flush_outliers_to_csv
...
GENERATE_MAPS = CONFIG.get("generate_percentile_maps", False)
...
def main():
    # ============================================================
    # ۰. ریست اجباری checkpoint به بلوک ۰
    # ============================================================
    reset_checkpoint(0, 0)
    logger.info("🔄 Checkpoint forcibly reset to block 0, station 0")
    ...
    logger.info("=" * 80)
    logger.info("🚀 CLIMATOLOGY ENGINE — FORCE RESTART (tmin & tmax only)")
    logger.info(f"   Years: {YEAR_START}–{YEAR_END} ({N_YEARS} years)")
    logger.info(f"   Days: {N_DAYS}")
    logger.info(f"   Variables: ['tmin', 'tmax'] (tmean skipped)")
    ...
    try:
        ...
        # ============================================================
        # ۴. checkpoint (auto_detect=False تا از ریست شده استفاده کند)
        # ============================================================
        ...
        # ============================================================
        # ۵. فقط tmin (0) و tmax (2)
        # ============================================================
        var_indices = [0, 2]   # tmin و tmax
        ...
        for block_idx in range(start_block, total_blocks):
            ...
            for var_idx in var_indices:
                ...
                logger.info(f"   🔄 Processing {var_name} for block {block_idx}")
                ...
                try:
                    ...
                    logger.error(f"   ❌ Error in {var_name} block {block_idx}: {e}")
                    save_checkpoint(block_idx, block_start)
                    raise

                try:
                    flush_outliers_to_csv(output_dir="outlier_reports", metadata_path="st_metadata.txt")
                except Exception:
                    pass
        ...
        ds.attrs["version"] = "3.6-force-restart"
