...
plt.plot(station_data[:365, 1], label='tmean', linewidth=2)
plt.plot(station_data[:365, 0], label='tmin', alpha=0.7)
plt.plot(station_data[:365, 2], label='tmax', alpha=0.7)
...
print(f" Mean (μ): {result['p1']:.3f}")
print(f" Std Dev (σ): {result['p2']:.3f}")
print(f" Log-likelihood: {result['loglik']:.3f}")
print(f" AICc: {result['aicc']:.3f}")
...
print(f"Loaded {len(plugins)} distributions: {[p.name for p in plugins.values()]}")
...
for code, dist in plugins.items():
    try:
        ...
        results[dist.name] = res
        print(f"{dist.name}: AICc = {res.get('aicc', np.nan):.3f}")
    except Exception as e:
        print(f"{dist.name}: Failed - {str(e)}")

# Find the best distribution (minimum AICc)
best_dist = min(results.items(), key=lambda x: x[1].get('aicc', np.inf))
print(f"\n✅ Best distribution: {best_dist[0]} (AICc = {best_dist[1]['aicc']:.3f})")
```
...
The engine calculates percentile-based confidence intervals:

$$\text{CI}_{95\%}(\theta) = [\theta_{(0.025)}, \theta_{(0.975)}]$$
...
Fazl Kazemi, A. (2025). ClimateProcessingEngine (Version 4.0) [Computer software].
GitHub. https://github.com/AminFazlKazemi/ClimateProcessingEngine
```
...
-   **GitHub:** <https://github.com/AminFazlKazemi>
